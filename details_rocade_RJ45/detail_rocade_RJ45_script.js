...
if (sessionStorage.getItem('id_equip_venant_baie') === null) {
    ...
    id_rocade = sessionStorage.getItem('nom_equip_venant_salle');
    id_rocade.split('-');
    id_rocade = id_rocade[0];

} else {

    id_rocade = sessionStorage.getItem('id_equip_venant_baie');
...
function detail() {
    ...
    fetch(`http://${iPort}/items/ROCADE_RJ45?filter[ID_ROCADE][_eq]=${id_rocade}`, requestOptions)
...
function construction_detail(resp) {
    ...
    delete tableau[6]//enlève l'id de la rocade
    delete tableau[7]//enlève l'id de la salle
    delete tableau[8]//enlève l'id de la baie  
